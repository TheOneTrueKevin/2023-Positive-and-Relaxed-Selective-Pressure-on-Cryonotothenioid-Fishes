	...
	case $1 in
		...
		--ProcessedPeptides)
			...
			shift # past argument
                        ...
                        ;;
		--CDS)
			CDS_DIR="$2" # where to find the cds files for building the CDS alignments
			# CDS do have to be preprocessed since naming conventions vary too much
			shift # past argument
                        shift # past value
                        ;;
		--CondensingBlastDB)
...
echo "CDS files (preprocessed) are in: ${CDS_DIR}"
...
# this script SHOULD also handle renaming...
#####################################################################
# needs to be tweaked so that this ALSO renames contigs appropriately...
...
#####################################################################
#subset.v02.py $WORKING_DIR $CDS_DIR fas
...
do
	...
	run_GUIDANCE="perl /home/iceboy/guidance.v2.02/www/Guidance/guidance.pl --seqFile ${FULLPATH}/${BASENAME}/${CLEANED_FILENAME} --msaProgram PRANK --seqType codon --outDir ${FULLPATH}/${BASENAME} --dataset ${BASENAME}_GUIDANCE --proc_num 25 --bootstraps 25"
...
do
	...
	cp "$WORKING_DIR"/"$BASENAME"/"$BASENAME".cleaned.cds "$WORKING_DIR"
